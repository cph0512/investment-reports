...
us=[]; filings=[]
concepts=['Revenues','RevenueFromContractWithCustomerExcludingAssessedTax','SalesRevenueNet','OperatingIncomeLoss','NetIncomeLoss']
for t,cik in CIK.items():
    try:
        sub=fetch_json(URLS['sec_sub'].format(cik=cik)); recent=sub.get('filings',{}).get('recent',{})
        rows=list(zip(recent.get('filingDate',[]), recent.get('form',[]), recent.get('accessionNumber',[]), recent.get('primaryDocument',[])))[:8]
        ...
        for con in concepts:
            ...
            for unit,arr in units.items():
                for a in arr:
                    if a.get('form') in ('10-Q','10-K') and a.get('fp') in ('Q1','Q2','Q3','FY') and a.get('val') is not None:
                        vals.append((a.get('filed',''), a.get('end',''), a.get('fp',''), con, unit, a.get('val')))
            if vals:
                vals=sorted(vals, reverse=True)
                metrics.append(vals[0])
...
upstream='07:45 上游 deep research job 失敗（ssh: Could not resolve hostname pindemac-mini.local），視為 stale/failed；本報未使用其內容，改以 TWSE/TPEx 官方 Open Data 與 SEC/公司 IR 快速補查。無法取得或未交叉驗證者明列資料缺口。'
summary_cards=[
 ('risk','上游資料 failed，已啟動 fallback',upstream,'Cron 注入紀錄 / 官方快查'),
...
parts.append(f'<div class="hero"><h1>航運物流股日報</h1><div class="meta">報告日期：{REPORT_DATE}　產出時間：{GENERATED}<br>涵蓋：前一台股交易日/公告日 {PREV_TW}；前一美股交易日/公告日 {PREV_US}<br><span class="stamp">資料 QC：上游 failed；未沿用上游，已官方 fallback 快查</span><br>{e(upstream)}</div></div>')
...
parts.append('</div></section>')
missing=[
 '上游 deep research 今日失敗，未能取得其產業新聞與人工深查；本報只使用可自動驗證的官方 fallback。',
